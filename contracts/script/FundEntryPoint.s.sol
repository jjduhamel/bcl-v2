...
// Tops up the Lobby paymaster's EntryPoint deposit. Split from Deploy so it can be re-run to refund.
...
contract FundEntryPoint is Script {
  ...
  function run() external {
    ...
    console.log('Funding Lobby paymaster:', address(lobby));
    ...
    lobby.depositToEntryPoint{ value: deposit }();
    vm.stopBroadcast();

    console.log('EntryPoint deposit:', lobby.entryPointDeposit());
  }
}
